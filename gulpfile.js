...
function runLighthouse(done) {
  ...
  // Check if the "Reports" folder exists, create it if not
  if (!fs.existsSync(reportsFolderPath)) {
    fs.mkdirSync(reportsFolderPath);
  }

  shell.task([
    `lighthouse http://localhost:3000 --preset=desktop --quiet --view=true --no-enable-error-reporting --output-path=${reportsFolderPath}/report-desktop-${timestamp}.html`,
    `lighthouse http://localhost:3000 --form-factor=mobile --quiet --view=true --no-enable-error-reporting --output-path=${reportsFolderPath}/report-mobile-${timestamp}.html`,
  ])();
  done();
}
...
exports.dev = series(cleanDist, copyFont, copyFavicons, copyDownloads, copyImages, compressImages, copyVideos, compileHTML, compileSCSS, cssVendor, jsVendor, compileJS, minifyScripts, resetPages, updateHtmlImageSrc, prettyHTML, browserSyncInit, watchFiles);
...
// PRODUCTION
exports.prod = series(cleanDist, copyFont, copyFavicons, copyDownloads, copyImages, compressImages, copyVideos, compileHTML, compileSCSS, cssVendor, minifyCSS, minifyCSS_RTL, jsVendor, compileJS, minifyScripts, renameSources_RTL, renameSources, prettyHTML, updateHtmlImageSrc, generateDocs, generateDevCSHTML, generateResxFilesEnglish, generateResxFilesArabic, generateExcel, autoTranslate, removeLocalizer, generateCriticalCSS, browserSyncInit, runLighthouse);

exports.productiondeploy = series(cleanDist, copyFont, copyImages, copyFavicons, compileHTML, compileSCSS, cssVendor, minifyCSS, minifyCSS_RTL, jsVendor, compileJS, renameSources_RTL, renameSources, prettyHTML, generateDocs, generateDevCSHTML, generateResxFilesEnglish, generateResxFilesArabic, generateExcel, autoTranslate, updateHtmlImageSrc, removeLocalizer, deploy, browserSyncInit);

exports.deployonly = series(deploy);
